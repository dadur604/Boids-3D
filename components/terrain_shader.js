...
export const Terrain_Shader = class Terrain_Shader extends Shader {
  // Terrain Shader acts just like a phong shader, but sets the terrain colors
  // based on the terrain height
    ...
    return (
      ...
      `
                ...
                void main(){     
                    ...
                    vec4 grass = vec4(0.4,0.9,0.4,1);
                    vec4 rock = vec4(0.5,0.5,0.5,1);
                    vec4 snow = vec4(1,1,1,1);

                    // angle from 0 to 1
                    float angle = max(0., dot(normal, position));
                    angle = smoothstep(0.999999, 1.0, angle);
                    
                    float grass_t = smoothstep(135.0, 138.0, height);
                    float snow_t = smoothstep(163.0, 165.0, height);
                    color = sand + grass_t * (grass - sand) + snow_t * (snow - grass);
